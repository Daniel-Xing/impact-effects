...
def print_energy(energy0, energy0_megatons):
    ...
    if energy_power == 0:
        ...
        discribe += (
            "Energy before atmospheric entry: %.2f Joules " % (energy0) + "/n"
        )
    else:
        print(
    ...
    if megaton_power == 0:
        if energy0_megatons < 1:
            ...
            discribe += (
                " = %.2f KiloTons TNT" % (energy0_megatons * 1000) + "/n"
            )
        else:
...
def print_change(vratio, mratio, lratio, trot_change, pratio):
    ...
    if mratio >= 0.01:
        ...
        discribe += (
            "%.2f percent of the Earth is melted" % (mratio * 100) + "/n"
        )

    if lratio >= 0.001:
        print("Depending on the direction and location the collision, ")
        discribe += (
            "Depending on the direction and location the collision, " + "/n"
        )
        if lratio < 0.01:
            print(
    ...
    if pratio >= 0.001:
        if pratio < 0.01:
            ...
            discribe += (
                "The impact shifts the Earth's orbit noticeably." + "/n"
            )
        elif pratio < 0.1:
            print("The imapct shifts the Earth's orbit substantially.")
            discirbe += (
                "The imapct shifts the Earth's orbit substantially." + "/n"
            )
        else:
        ...
        discribe += (
            "The impact does not shift the Earth's orbit noticeably." + "/n"
        )
...
def print_ejecta(
    ...
    if distance * 1000 <= 3 * cdiameter / 2:
        ...
        discribe += (
            "Your position is beneath the continuous ejecta deposit." + "/n"
        )
    else:
        print(
...
def print_thermal(
    ...
    if B >= 0.1:
        ...
        discribe += (
            "The fireball appears %f times larger than the sun" % (B) + "/n"
        )
    ...
    if thermal_power == 0:
        ...
        discribe += (
            "Thermal Exposure:  %.2f Joules/m<sup2" % (thermal_exposure) + "/n"
        )
    else:
        print(
    ...
    discribe += (
        "Duration of Irradiation:  %f %s" % (irradiation_time, "seconds")
        + "/n"
    )
    ...
    if flux >= 15 and flux <= 25:
        ...
        discribe += (
            " (Flux from a burner on full at a distance of 10 cm)" + "/n"
        )
    print("")
...
def print_seismic(magnitude, seismic_arrival):
    ...
    if seismic_arrival < 0.1:
        ...
        discribe += (
            "The major seismic shaking will arrive almost instantly." + "/n"
        )
    else:
        print(
    ...
    if magnitude >= 9.5:
        ...
        discribe += (
            " (This is greater than any earthquake in recorded history)" + "/n"
        )
...
def print_tsunami(
    ...
    if WaveAmplitudeLowerLimit > 0.1:
        ...
        print("Tsunami wave amplitude is less than 10 cm at your location. ")
        discribe += (
...
def print_crater(
    ...
    discribe += (
        "%f %s ( = %f %s )" % (Dtr, "meters", Dtr * 3.28, "feets") + "/n"
    )
    print(
    ...
    print(
        "%f %s ( = %f %s )" % (cdiameter, "meters", cdiameter * 3.28, "feets")
    )
    discribe += (
        "%f %s ( = %f %s )" % (cdiameter, "meters", cdiameter * 3.28, "feets")
        + "/n"
    ...
    if mcratio >= 1:
        ...
        discribe += (
            "Melt volume = %f times the crater volume\n" % (mcratio) + "/n"
        )
