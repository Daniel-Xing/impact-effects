...
def print_thermal(
    ...
    if thermal_power == 0:
        print(
            "Thermal Exposure:  %.2f Joules/m<sup2"
            % (thermal_exposure)
        )
    else:
        print(
...
def print_crater(vMelt, Dtr, depth, wdiameter, pdiameter, dispersion,
                 collinsIFactor, depthtr, mratio, mcratio, cdiameter, depthfr, brecciaThickness, velocity):
    ...
    if depth != 0:
        print("The crater opened in the water has a diameter of %f %s ( = %f %s )." % (
              wdiameter, "meters",
              wdiameter * 3.28,
              "feets",
              ))
    ...
    if pdiameter < 1000:
        if dispersion >= Dtr:
            print("The result of the impact is a crater field, not a single crater.  The following dimensions are for the crater produced by the largest fragment.")

        elif collinsIFactor < 1:
            print("Crater shape is normal in spite of atmospheric crushing fragments are not significantly dispersed.")

    # Report transient crater dimensions
    print("Transient Crater Diameter:")

    print("%f %s ( = %f %s )" % (Dtr, "meters", Dtr * 3.28, "feets"))
    print("Transient Crater Depth:  %f %s ( = %f %s )" %
    ...
    print("Final Crater Depth:  %f %s ( = %f %s )" % (
          depthfr, "meters", depthfr*3.28, "feets"))

    # Report the type of crater formed
    if mcratio >= 1:
        print("The final crater is replaced by a large, circular melt province.")
    elif cdiameter >= 3200:
        print("complex crater .")
    else:
        print("simple crater ")
        print("The floor of the crater is underlain by a lens of broken rock debris (breccia) with a maximum thickness of %f %s ( = %f %s )." % (
    ...
    if velocity >= 12 and mratio < 0.1:
        ...
        # Melt volume
        if vMelt < 0.001:
            print("The volume of the target melted or vaporized is %f m^3 = %f feet^3" % (
                  vMelt * 10**9, vMelt * 10**9 * 3.28**3))
        else:
            print("The volume of the target melted or vaporized is %f km^3 = %f miles^3" %
                  (vMelt, vMelt * 0.2399))
        ...
        if mcratio < 1:
            ...
            if Dtr >= 3200:
                print(", where its average thickness is %f %s ( = %f %s ). \n" % (
                      melt_thickness*1000, "meters", melt_thickness*1000*3.28, "feets"))
        else:
            print(
